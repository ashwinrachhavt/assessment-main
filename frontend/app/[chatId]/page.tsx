...
export default function Home({ params }: { params: { chatId: string } }) {
  ...
  const { data: formsData, mutate: mutateForms } = useSWR({ url: `chat/${params.chatId}/forms` }, fetcher)
          ...
          <div className="space-y-3">
            {forms.map((form: any) => {
              const submittedAt = form?.created_at
                ? new Date(form.created_at.endsWith("Z") ? form.created_at : form.created_at + "Z").toLocaleString()
                : "Unknown"
              ...
              return (
                <div key={form.id} className="rounded-md border border-neutral-200 bg-white p-3 shadow-sm">
                  {isEditing ? (
                    // TASK 2: Edit mode
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) => setEditFormData({ ...editFormData, status: e.target.value ? parseInt(e.target.value) : null })}
                          className="w-full text-sm border border-gray-300 rounded px-2 py-1"
                        >
                    ...
                    // TASK 1 & 2: View mode
                    <div>
                      <div className="text-sm"><span className="font-semibold">Name:</span> {form.name}</div>
                      <div className="text-sm"><span className="font-semibold">Email:</span> {form.email}</div>
                      <div className="text-sm"><span className="font-semibold">Phone:</span> {form.phone_number}</div>
                      <div className="text-sm"><span className="font-semibold">Status:</span> {statusLabel}</div>
